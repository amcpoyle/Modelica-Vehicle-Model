model Wheel22
    ...
    constant Real car_mass = ModelConstants.carMass;
    constant Real tire_static_mass = car_mass*ModelConstants.weight_dist_front*ModelConstants.weight_dist_left;
    ...
    // Real LLT(start=0); TODO: not sure if I need this right now
...
equation
    ...
    // LLT = (ay*cg_height)/t;
    Fz22 = 0.5*(((car_mass*g_n*a2)/car_length) + (0.5*rho*S*Cz)*(vx^2) + ((car_mass*ax*cg_height)/car_length)) + ((car_mass*ay*cg_height)/car_length); // last term is simplified LLT, no susp
    Fy22 = MagicFormula(d0=d0_fy, d1=d1_fy, c=c_fy, b0=b0_fy, b1=b1_fy, x=alpha, y=Fz22);
    Fx22 = MagicFormula(d0=d0_fx, d1=d1_fx, c=c_fx, b0=b0_fx, b1=b1_fx, x=k, y=Fz22);
